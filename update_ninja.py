...
ninja_idx = c.find('def analyze_ninja_scalper')
return_idx = c.find('    return {', ninja_idx + 100)
end_idx = c.find('    }', return_idx) + 6
...
if return_idx != -1 and end_idx != -1:
    c = c[:return_idx] + enhancement_ninja + c[end_idx:]

with open(r'app\services\engines\technical_engine.py', 'w', encoding='utf-8') as f:
    f.write(c)

print('Fixed ninja scalper end return')
